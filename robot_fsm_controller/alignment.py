...
class Alignment(Node):
    ...
    def __init__(self):
        ...
        ## 
        self.alignment_start_time = None  # This will be set when alignment is activated.
        self.alignment_timeout_duration = Duration(seconds=10)
        ##
    ...
    def thermal_sub_callback(self, msg):
        ...
        if not self.alignment_active:
            return  # Do nothing if alignment is not currently active

        # Timeout check
        if self.alignment_start_time and self.get_clock().now() - self.alignment_start_time > self.alignment_timeout_duration:
            self.get_logger().warn("Alignment timed out. Resuming exploration...")
            self.alignment_active = False
            twist = Twist()
            twist.linear.x = 0.0
            twist.angular.z = 0.0
        ...
        ALIGNMENT_COMPLETE_THRESHOLD = 40
        ...
        if difference < ALIGNMENT_COMPLETE_THRESHOLD:
            ...
            if min(self.difference_list) == difference:
                ...
                if direction == 1:  # left is hotter; 
                    twist.linear.x = 0.0
                    twist.angular.z = -0.3
                elif direction == -1:  # right is hotter; 
                    twist.linear.x = 0.0
                    twist.angular.z = 0.3
    ...
    def is_alignment_complete(self, activate):
        ...
        self.IsAlignmentComplete.finish = activate
