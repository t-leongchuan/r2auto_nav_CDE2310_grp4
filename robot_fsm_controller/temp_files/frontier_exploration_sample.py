...
## Frontier Exploration is a Node unlike Frontier Search.
#
# Summary:
#
#
...
class FrontierExploration:
    def __init__(self):
        ...
        # Publishers
        # Publishes a Path, which consists of:
        # 1. Header (Timestamped Information)
        # 2. An array of PoseStamped (variable name is called poses)
        #
        #
        # PoseStamped consists of a Pose (Point, Quaternion) + Timestamped details
        #
        # Essential information that represents a path for robot to follow
        self.pure_pursuit_pub = rospy.Publisher(
            "/pure_pursuit/path", Path, queue_size=10
        )

        # Optional But might be good when implementing
        if self.is_in_debug_mode:
            # GridCells Consists of:
            # 1. Header (Timestamped information)
            # 2. cell_width (float 32 bits)
            # 3. cell_height (same)
            # 4. Point[] cells 
            # Recall that a Point is just (x,y,z) coordinates.
            ...
            # c space is configuration space.
    ...
    # Just use the code i've used to replace this
    ...
    # This is not required and probably just for testing purposes.
    #
    #
    # OPTIONAL???
    def save_map(self):
